...
describe("Test de RUTAS",()=>{
    describe("GET /rickandmorty/character/:id",()=>{
        ...
        it('Si hay un error responde con status: 500', async ()=>{
            await agent.get('/rickandmorty/character/8881').expect(500);
        });
    ...
    describe("GET /rickandmorty/login", ()=>{
        it("La información de login es correcta", async ()=>{
            const response = (await agent.get("/rickandmorty/login?email=kenrodrigoariel@gmail.com&password=q1w2e3r4")).body
            expect(response.access).toEqual(true)
        });

        it("La información de login es incorrecta", async ()=>{
            const response = (await agent.get("/rickandmorty/login?email=kenchurodrigoariel@gmail.com&password=r4e3w2q1")).body
            expect(response.access).toEqual(false)
        });
    });

    describe("POST /rickandmorty/fav", ()=>{
        const character1 = {id: "1", name: "Ken"}
        const character2 = {id: "2", name: "Mica"}
    ...
    describe("DELETE /rickandmorty/fav/:id",()=>{
        ...
        it("Devuelve el arreglo correspondiente si no se elimina ningún personaje", async ()=>{
            const response = (await agent.delete("/rickandmorty/fav/98731")).body;
            expect(response).toContainEqual(character1)
            expect(response).toContainEqual(character2)
        });
